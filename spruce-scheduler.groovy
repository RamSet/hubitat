 /**
 *  Spruce Scheduler
 *  IMPORT URL: https://raw.githubusercontent.com/RamSet/hubitat/main/spruce-scheduler.groovy
 *
 ...
 *    Hubitat) with Open-Meteo (https://open-meteo.com — free, no API key).
 *    isWeather() is unchanged: the new helpers synthesize a response
 *    matching the TWC daypart/conditions field shapes, including
 *    sunriseTimeLocal / sunsetTimeLocal in the same parseable format.
 *  - Uses location.latitude / location.longitude from the hub's
 *    configured location for the forecast query. Set location under
 *    Hubitat Settings → Location.
 *  - 15-minute response cache (state.__omCacheData/At) so the three
 *    isWeather() calls (city/conditions/forecast) hit the API once.
 *  - getCity() now returns location.name (the hub label) instead of
 *    a TWC-derived city, since Open-Meteo's forecast endpoint doesn't
 *    expose a geocoded city.
 *
 *  -------v2.56 (lgkahn) -----------
 *  - add weather device option to get precip from
 *
 ...
 * ported to hubitat 9/4 lgk kahn@lgk.com
 */
...
def startPage(){
    dynamicPage(name: 'startPage', title: 'Spruce Smart Irrigation setup', install: true, uninstall: true)
    {
        ...
        section(''){
            href(title: 'Scheduler Version 2.56',
    ...
    dynamicPage(name: 'globalPage', title: '') {
        ...
        section('Push Notifications') {   
            input(name: 'notify', type: 'enum', title: 'Select what push notifications to receive.', required: false,
                    multiple: true, options: ['Daily', 'Delays', 'Warnings', 'Weather', 'Moisture', 'Events'])
...
def zoneSetPage() {
    dynamicPage(name: 'zoneSetPage', title: "Zone ${state.app} Setup") {
        ...
        section(''){
             ...
             input "zone${state.app}", 'enum', title: 'Sprinkler Type', multiple: false, required: false, defaultValue: 'Off', submitOnChange: true, options: ['Off', 'Spray', 'Rotor', 'Drip', 'Master Valve', 'Pump']
         }
         ...
         section(''){
            href(name: 'toplantSetPage', title: "Landscape Select: ${setString('plant')}", required: false, page: 'plantSetPage',
             ...
             input "plant${state.app}", 'enum', title: 'Landscape', multiple: false, required: false, submitOnChange: true, options: ['Lawn', 'Garden', 'Flowers', 'Shrubs', 'Trees', 'Xeriscape', 'New Plants']
            }
        ...
        section(''){
            ...
            input "option${state.app}", 'enum', title: 'Options', multiple: false, required: false, defaultValue: 'Cycle 2x', submitOnChange: true,options: ['Slope', 'Sand', 'Clay', 'No Cycle', 'Cycle 2x', 'Cycle 3x']
            }
...
private String getaZoneSummary(int zone){
 //   log.debug "in get a zone summary zone = $zone"
    ...
    if (days && (days.contains('Even') || days.contains('Odd'))) {
      ...
      //  log.debug "passing to getrundays $dpwMap"
...
def installSchedule(){
   ...
   // if (settings.isRain || settings.isSeason) schedule("${randomSeconds} 57 23 1/1 * ? *", getRainToday)        // capture today's rainfall just before midnight
    ...
    if (settings.switches && settings.startTime && settings.enable){
       ...
       // log.debug "starttime = $settings.startTime, tz = $location.timeZone time = $getTime() starttimestring = $startTimeString()"
       ...
       // log.debug "calling schedule checktinme $checktime"
        
       
        
        schedule(settings.startTime, preCheck)
...
// Get string of run days from dpwMap
private String getRunDaysorig(day1,day2,day3,day4,day5,day6,day7)
{
    String str = ''
    if(day1) str += 'M'
    if(day2) str += 'T'
...
// -------------------------------------------------------------------------
// The original Plaid scheduler used Hubitat's built-in getTwcLocation /
// getTwcConditions / getTwcForecast helpers, which proxied to IBM's The
// Weather Channel API. Hubitat retired those helpers when IBM/TWC pulled
// access, so every call now 404s and isWeather() falls through to the
// "Please check Zipcode/PWS setting, error: null" branch.
//
// Replacement: Open-Meteo (https://open-meteo.com). Free, no API key,
// global coverage, daily forecast for the next 7 days with precipitation
// sum/probability, max temperature, sunrise/sunset, plus hourly humidity
// that we aggregate to daily means. Uses location.latitude / longitude
// directly from the hub's configured location.
//
// We synthesize a response shape that matches the TWC fields read by
// isWeather() so that downstream code is unchanged:
//   conditionsData.precip24Hour          (numeric string, inches)
//   conditionsData.sunriseTimeLocal      (ISO timestamp parseable by
//   conditionsData.sunsetTimeLocal        "yyyy-MM-dd'T'HH:mm:ss-SSSS")
//   forecastData.daypart[0].daypartName[0]       (always "Today")
//   forecastData.daypart[0].qpf[i]               (inches; per half-day)
//   forecastData.daypart[0].precipChance[i]      (POP %)
//   forecastData.daypart[0].temperature[i]       (F, high)
//   forecastData.daypart[0].relativeHumidity[i]  (%)
//
// TWC's daypart layout had two entries per day (day half + night half).
// Open-Meteo returns daily totals, so each day's value is duplicated into
// both half-day slots (qpf[2*i] and qpf[2*i+1]). This keeps the original
// isWeather() index math working — including the qpf[1]/temperature[2]
// "tomorrow" lookups — without touching that function.
// =========================================================================

// Last successful response cached in state to skip repeat fetches within
// the same isWeather() pass (getCity, getConditions, getForecast all call
// the same endpoint).
private Map omFetch() {
    BigDecimal lat = (location?.latitude  ?: 0.0) as BigDecimal
    BigDecimal lon = (location?.longitude ?: 0.0) as BigDecimal
    if (lat == 0 && lon == 0) {
        log.warn "Open-Meteo: hub location not configured (Settings → Location). Cannot fetch weather."
        return null
    }
    Map params = [
    ...
    try {
        ...
        httpGet(params) { resp ->
            if (resp?.status == 200 && resp.data) {
                result = resp.data instanceof Map ? resp.data : new groovy.json.JsonSlurper().parseText(resp.data.text)
            } else {
                log.warn "Open-Meteo: HTTP ${resp?.status}"
            }
        }
        return result
    } catch (e) {
...
// Compute daily means of hourly relative_humidity_2m for each YYYY-MM-DD,
// returned as a map keyed by ISO date.
private Map<String, Integer> omDailyHumidity(Map om) {
    Map<String, List<Number>> perDay = [:]
    def times = om?.hourly?.time
    def hums  = om?.hourly?.relative_humidity_2m
    if (!times || !hums) return [:]
    times.eachWithIndex { String ts, int i ->
        String day = ts?.substring(0, 10)
        Number h = (i < hums.size()) ? hums[i] : null
        if (day && h != null) {
            (perDay[day] = perDay[day] ?: []) << h
        }
    }
    Map<String, Integer> means = [:]
    perDay.each { day, vals ->
        means[day] = ((vals.sum() as BigDecimal) / vals.size()).setScale(0, BigDecimal.ROUND_HALF_UP).toInteger()
    }
    return means
}

// Sum hourly precipitation for the last ~24h ending now. Falls back to
// today's daily precipitation_sum if hourly data is missing.
private float omPrecipLast24h(Map om) {
    def times = om?.hourly?.time
    def precs = om?.hourly?.precipitation
    if (!times || !precs) return 0.0f
    Date now = new Date()
    long cutoffMs = now.getTime() - (24L * 60L * 60L * 1000L)
    float total = 0.0f
    times.eachWithIndex { String ts, int i ->
        try {
            Date t = Date.parse("yyyy-MM-dd'T'HH:mm", ts)
            if (t.getTime() >= cutoffMs && t.getTime() <= now.getTime()) {
...
// Build a TWC-shaped synthetic response. Returns a Map with `conditions`
// and `forecast` sub-maps used by getConditions()/getForecast().
private Map omSynthesise() {
    Map om = state.__omCacheData
    Long cachedAt = state.__omCacheAt as Long
    long nowMs = now()
    // 15-minute cache: a single isWeather() invocation makes three calls
    // (city, conditions, forecast); scheduler runs at most a few times a
    // day, so caching at 15 min keeps under Open-Meteo's free-tier limits.
    ...
    Map<String, Integer> humByDay = omDailyHumidity(om)
    def daily = om.daily
    if (!daily?.time) return null

    // past_days=1 puts yesterday at index 0; find today's index by matching
    // today's local YYYY-MM-DD against daily.time. Fall back to index 1 (the
    // typical position with past_days=1) if matching fails.
    String todayISO = new Date().format("yyyy-MM-dd", location?.timeZone ?: TimeZone.getDefault())
    int todayIdx = daily.time.findIndexOf { it == todayISO }
    if (todayIdx < 0) todayIdx = (daily.time.size() > 1) ? 1 : 0

    // Build interleaved (day-half + night-half) arrays matching TWC daypart layout.
    List qpf = []
    List precipChance = []
    List temperature = []
    ...
    // (yyyy-MM-dd'T'HH:mm:ss-SSSS). Open-Meteo returns "YYYY-MM-DDTHH:MM"
    // in local time (timezone=auto). We append :00-0000 so the parse
    // succeeds; only HH:mm get used downstream.
    String sunrise = (daily.sunrise && daily.sunrise[todayIdx]) ? "${daily.sunrise[todayIdx]}:00-0000" : null
    String sunset  = (daily.sunset  && daily.sunset[todayIdx])  ? "${daily.sunset[todayIdx]}:00-0000"  : null

    float p24 = omPrecipLast24h(om)
    String precip24Str = String.format(Locale.US, "%.2f", p24)

    Map conditions = [
        precip24Hour: precip24Str,
    ...
    return [conditions: conditions, forecast: forecast]
}

def getCity() {
    String fallback = (location?.name ?: zipString() ?: "home")
    // Open-Meteo doesn't return a city name in /v1/forecast. Use the hub's
    // location name (set in Hubitat Settings → Location) so the weather
    // report shows a meaningful label instead of "unknown city".
    return fallback
}

def getConditions() {
    Map synth = omSynthesise()
    return synth?.conditions
}
...
def getDPWDays(int dpw){
   // log.debug "in getdpwdays dpw = $dpw" 
