 ...
 *
 * Changelog:
 *  v1.3.3 - Fixes namespace to match the HPM manifest (was "Mezel"), so updates
 *           and Package Manager matching work correctly.
 *           Timestamps now use the hub's timezone instead of a hardcoded zone.
 *
...
private formatTimestamp(value, name) {
    ...
    try {
        ZonedDateTime zdt
        def defaultZone = location?.timeZone?.toZoneId() ?: java.time.ZoneId.of("America/Denver")
